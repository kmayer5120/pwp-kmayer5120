...
const handlePostRequest = (request, response) => {
  ...
  if (request.recaptcha.error) {
    return response.send(
      `<div class="alert alert-danger" role='alert'><strong>There was an error with recaptcha!</strong></div>`
    );
  }
  ...
  if (errors.isEmpty() === false) {
    ...
    return response.send(
      `<div class="alert alert-danger" role="alert"><strong>${currentError.msg}</strong></div>`
    );
  }
  ...
  mg.messages().send(mailgunData, (error) => {
    if (error) {
      return response.send(
        Buffer.from(
          `<div class='alert alert-danger' role='alert'>Unable to send email!</div>`
        )
      );
    }
  });
